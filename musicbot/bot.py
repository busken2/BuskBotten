...
#######################################################################################################################
    ...
    def _gen_embed(self):
        ...
        e = discord.Embed()
        e.colour = 7506394
        e.set_footer(text='busken2/BuskBotten ({})'.format(BOTVERSION), icon_url='https://cdn.discordapp.com/avatars/162577953442627584/445eeb3141b5caa6ed0b03d4114e72c3.png?size=128')
        e.set_author(name=self.user.name, url='https://github.com/busken2/BuskBotten', icon_url=self.user.avatar_url)
        return e
    ...
    async def cmd_help(self, message, channel, command=None):
        ...
        desc = '```\n' + ', '.join(self.commands) + '\n```\n' + self.str.get(
            'cmd-help-response', 'For information about a particular command, run `{}help [command]`\n'
                                 'For further help, contact busken#6478').format(prefix)
        if not self.is_all:
